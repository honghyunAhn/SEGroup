...
//SEGROUP과 Changbi를 통합하면서 PayController가 2개가 있을 필요가 없으며, 2개 있을시 에러 발생하므로 주석처리
//@Controller
public class PayController {
 
    public Model payInfo(Model model, HttpServletRequest request, String price, String goodname,String pay_seq, String from, String to)
             {
    	...
    	try{
        String mid = "softenjin0"; // 가맹점 ID(가맹점 수정후 고정)
 
        // 인증
        String signKey = "amVtb3M2SGE2NC9EeHVWUHBxTmtCdz09"; // 가맹점에 제공된 웹
                                                                // 표준
                                                                // 사인키(가맹점
                                                                // 수정후 고정)
      ...
      //  String price = price; // 상품가격(특수기호 제외, 가맹점에서 직접 설정)
        ...
        model.addAttribute("mid", mid); // 가맹점 ID
        model.addAttribute("oid", oid); // 가맹점+ 주문번호 + TimeStampe
        model.addAttribute("price", price); 
        model.addAttribute("timestamp", timestamp); 
        model.addAttribute("signature", signature); // oid+price+timestamp  key,value형식으로 SHA-256 Hash로 
        model.addAttribute("mKey", mKey); // 가맹점에 제공된 웹 표준 사인키  인증할떄 사용하는데 SHA-256 암호화 
        model.addAttribute("cardQuotaBase", cardQuotaBase); // 가맹점 사용할 할부개월수 
        model.addAttribute("cardNoInterestQuota", cardNoInterestQuota);
        model.addAttribute("siteDomain", siteDomain);
        model.addAttribute("goodname", goodname); // 상품명 
        model.addAttribute("offerPeriod",from+"-"+to); 
        model.addAttribute("seq",pay_seq);
        /**********************************************************************************************
         * INIpay 모듈 세팅 종료
         **********************************************************************************************/
    ...
    public HashMap<String, String> makeSignature(HashMap<String, String> param){
      ...
      //  String price = price; // 상품가격(특수기호 제외, 가맹점에서 직접 설정)
        ...
        resultMap.put("cardQuotaBase", cardQuotaBase);
        resultMap.put("cardNoInterestQuota", cardNoInterestQuota);
    ...
    /*public String payAfter(Model model, HttpServletRequest request,
        ...
        *//**********************************************************************************************
        ...
        SocietyEduUserViewController se=new SocietyEduUserViewController();
 
        return se.user_up(model,auth);

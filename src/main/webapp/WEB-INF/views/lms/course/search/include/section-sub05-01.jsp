	...
	$(function(){
		...
		function getCourseList(parent_cate_opt, cate_opt, search, display_list){
			...
			$.ajax({
				...
				dataType : "json",
				type	: "get",
				url		: "/lms/course/search/selectSearchCourseList",
				data    : {
					"grand" : '',
					"startPage" : startPage,
					"keyWord" : search,
				...
				success	: function(data) {
					...
					$.each(data, function(index, courseList){
						...
						if(courseList.grand_name != null){
							content += '<li class="class-list d-flex justify_between' + ' load-sub01-01-01">';
							content += 		'<div class="c-ctt c-category">';
							content += 					'<div class="class-title-wrap">';
							content += 				'<p class="grandName" data-value="'+ courseList.grand_name+ '" style="margin-left: 35px;">' + courseList.grand_name+ '</p>';
							content += 		'</div>';
							content += 		'</div>';
							content +=		'<div class="c-ttl subject">';
							content += 			'<a class="d-flex courseDetail">';
							...
							if(courseList.icon_displays !== undefined){
							...
							content += 						'</div>';
							content += 						'<div class="title text-over txt-ov-2" data-value="' + courseList.course_id + '">' + courseList.name + '</div>';
							content += 					'</div>';
							content += 			'</a>';
							content += 		'</div>';
							...
							content += 		'</div>';
							content += 		'<div class="c-ctt">';
							/* 학습기간에 대한 처리가 필요함 */						
							content += 			'<p class="term" data-value="'+ courseList.selp_period +'">' + courseList.selp_period +'일</p>'; 
							/* 학습기간에 대한 처리가 필요함 */						
							content += 		'</div>';
							...
							if(courseList.tasting_url){
								...
								content += 		'<div class="c-ctt">';
								content +=		'	<a class="btn_default btn_xsmall" onclick="document.getElementById(\'oneplayform'+index+'\').submit();">맛보기</a>';
								content += 		'</div>';
								content += 		'</form>';
							}else{
								content += 		'<div class="c-ctt">';
								content +=		'<a class="btn_default btn_xsmall" onclick="alert(\'맛보기가 설정되어 있지 않습니다.\')">맛보기</a>';
								content += 		'</div>';
							}
							content += 		'<div class="c-ctt"><a class="btn_point btn_xsmall modal-popup">수강신청</a>';						
							content += 		'</div>';
							content += '</li>';
						}
					});
					...
					$('.courseDetail').on('click', function(){
						var grand = $(this).parent().parent().children('.c-category').children().children('.grandName').html();
						var course_id = $(this).children('.class-title-wrap').children('.txt-ov-2').attr('data-value');
						document.location.href = "/lms/course/search/movePage?course_id=" + course_id + "&grand=" + grand;
				    ...
				    $('.modal-popup').click(function() {
				    	...
				    	/* 팝업정보 세팅 */				    	
				    	var classNm = $(this).parent().parent().children('.subject').find('div.txt-ov-2').html();
						var courseId = $(this).parent().parent().children('.subject').find('div.txt-ov-2').attr('data-value');
				    	var classPrice = $('.c-price').children('.classPrice').html();
				    	var realPrice = $('.c-price').children('.classPrice').attr('data-value');
/* 				    	var discountPrice = $(this).parent().parent().children('.c-price').children('.tableWrap').children('.discountPrice').html(); */
				    	var learnPeriod =  $('.term').attr('data-value');
...
<!-- 			<button type="button" class="button btn-wh img-icon search" id="searchBtn"></button> -->
	...
	<div class="class-listWrap">
		...
		<ul class="class-list-title d-flex">
			<li class="c-ttt" style="margin-left: 40px;">구분</li>
			<li class="c-ttl" style="width: 500px; margin-left: 60px;">과정명</li>
			<li class="c-ctt">교육비</li>
			<li class="c-ctt">학습기간</li>
			<li class="c-ctt">맛보기</li>
			<li class="c-ctt">수강신청</li>
		</ul>
		<!-- 과정목록 : 내용
                    - 목록 추가시 하위카테고리 분류하여

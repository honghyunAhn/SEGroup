...
function searchList(credit) {
	if(typeof(credit) == 'undefined') {
		credit = $('#moveForm input[name="credit"]').val();
	}
	var sort = $('#courseSort button.on').data('sort') || '';
	location.href = '<c:url value="/learn/course/grades"/>?credit='+credit+'&sort='+sort;
}
</script>
		...
		<div class="body">
			<div class="sub-header">
				<p class="sub-nav">
					...
					<a href="#">연수 신청</a>
					<a class="nth-last" href='${contextPath}'>학점별 연수</a>
				</p>
				<h2 class="sub-title">학점별 연수 </h2>
			...
			<div class="sub-body">
				...
				<div class="tab-group">
					<ul class="tab-header sub x5">
						<li><a class='nth-1st <c:if test="${'0' eq param.get('credit')}">on</c:if>' href="javascript:searchList('0')">전체 보기</a></li>
						<li><a class='<c:if test="${'4' eq param.get('credit')}">on</c:if>' href="javascript:searchList('4')">4학점</a></li>
						<li><a class='<c:if test="${'3' eq param.get('credit')}">on</c:if>' href="javascript:searchList('3')">3학점</a></li>
						<li><a class='<c:if test="${'2' eq param.get('credit')}">on</c:if>' href="javascript:searchList('2')">2학점</a></li>
						<li><a class='<c:if test="${'1' eq param.get('credit')}">on</c:if>' href="javascript:searchList('1')">1학점</a></li>
					</ul>
					<div class="tab-body">
						<div class="list-top">
						...
						<table id="list-learn" class="list">
							...
							<c:if test="${empty list}">
								<tr>
									<td colspan="7" class="nth-1st">조회된 내용이 없습니다.</td>
								</tr>
							</c:if>
							</tbody>
						</table>

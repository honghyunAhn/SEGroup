	...
	myApp.controller('AdminController', ['$scope','$compile','$http', function($scope,$compile,$http){
		...
		$scope.list_draw = function(data){
			...
			$.each(data, function(index,item){
				...
				context += '<td><button ng-click="final_commit(\''+item.fap_jobfair_final_decision_time_gb+'\',\''+item.fap_jobfair_divide_seq+'\')">적용</button></td>';
				context += '</tr>';
			});
...
<body  ng-app="myapp" ng-controller="AdminController">
	...
	<input type="button" id="insertBtn" value="등록폼">
	<input type="button" id="deleteBtn" ng-click="setting_delete()" value="삭제">
	<input type="button" ng-click="compPassBtn()" value="기업전달">
	
	<div id="insertDiv" style="display: none;">
		<table class="search_box">
				<tr>
					<th>예비합격자 확정 선택 여부</th>
					<td>
						<select name="fap_jobfair_final_decision_time_gb" id="fap_jobfair_final_decision_time_gb">
			...
			최종확정기간 시작일<input type="text" id="jobfair_st" name="fap_jobfair_final_decision_time_st"> ~ 최종확정기간 종료일<input type="text" id="jobfair_et" name="fap_jobfair_final_decision_time_et"> <br>
			설명문구<br>
			<textarea rows="4" cols="100" id="fap_jobfair_final_decision_time_ct" name="fap_jobfair_final_decision_time_ct"></textarea>
		<input type="button" id="finalInsertBtn" ng-click="final_insert()" value="등록">
	</div>

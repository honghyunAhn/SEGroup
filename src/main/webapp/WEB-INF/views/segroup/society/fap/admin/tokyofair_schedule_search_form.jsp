...
<script type="text/javascript">
	...
	function percentage(){
		var dt = $("#fap_tokyofair_date_dt").val();
		var total = $("."+dt).find(".c_td").length;
		var merged = $("."+dt).find(".merged").length;
		var hidden = $("."+dt).find(".hidden").length;
		
		var per = (merged + hidden) / total * 100;
		$("#room_per").html(per.toFixed(2)+"%");
	}
	...
	myApp.controller('TokyoFairController', ['$scope','$compile','$http', function($scope,$compile,$http){
		...
		$(".btn_print").on("click",function(){
			...
			window.open('','tokyofair_schedule_print',"width=1200,height=2000,left='+popupX+',top='+popupY+',screenX='+popupX+',screenY='+popupY+',toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=yes,scrollbars=no,copyhistory=no,scrollbars=yes");
		...
		$(".c_td").off().on("click",function(index,item){
			...
			if($elm.hasClass("merged")) {
				...
				if($elm.attr("fap_tokyofair_screening_gb") == 'E0502' || $elm.attr("fap_tokyofair_screening_gb") == 'E0503' || $elm.attr("fap_tokyofair_screening_gb") == 'E0504'){
		...
		$scope.screening_callback = function(){
			...
			$http({
				...
				$.each(response.data, function(index,item){
					...
					context +='<code value="'+item.fap_job_ad_category_type+'"></code>';
					context +='</div>';
					context +='</div>';
				});
				
				$(".layer-message .member-message").html($compile(context)($scope));
		 ...
		 $scope.drawSchedule = function(tokyofairSch){
			 ...
			 $.each(tokyofairSch, function(index,item){
				...
				if(item.fap_tokyofair_screening_gb == 'E0500'){
					...
					if(typeof item.fap_tokyofair_screening_admin_req != 'undefined' && item.fap_tokyofair_screening_admin_req != ''){
						context += '<div class="merged-admin-req" title="'+item.fap_tokyofair_screening_admin_req+'">';
						context += '요청사항';
						context += '</div>';
					}
					if(typeof item.fap_tokyofair_screening_user_req != 'undefined'  && item.fap_tokyofair_screening_user_req != ''){
						context += '<div class="merged-user-req" title="'+item.fap_tokyofair_screening_user_req+'">';
						context += '전달사항';
						context += '</div>';
					}
					context += '</div>';
...
<div class="tokyofair-container">
	<div class="tokyofair-wrap">
		...
		<h4>
			<span>면접실 접유율</span><span id="room_per"></span> <br>
		</h4>	
...
<div class="dim-layer">
    <div class="dimBg"></div>
    <div id="schedule_list_layer" class="pop-layer scroll">
            ...
            <div class="pop-conts">
            	...
            	<div class="layer-message">
            		...
            		<div class="list-message">
            			<div>
	            			<select id="admin-schedule-type">
	            				<option value="0">선택해 주세요</option>
	            				<option selectcode value="E0502"></option>
	            				<option selectcode value="E0503"></option>
	            				<option selectcode value="E0504"></option>
	            			</select>
            			</div>
            			<div>
            				<select id="admin-schedule-time">
            					<option value="0">선택해 주세요</option>
        ...
        <div class="pop-container">
            <div class="pop-conts">
                ...
                <div class="btn-r">
                    ...
                    <a href="javascript:void(0);" class="btn-layerDelete confirm-next">Delete</a>
                </div>
                <!--// content-->
            </div>
        </div>
...
<form action="/fap/admin/tokyofair_schedule_print_form" method="post" name="tokyofair_schedule_print" id="tokyofair_schedule_print">

...
const PW_MAX_LENGTH = 40;
	...
	$(function() {
		...
		_update = _.debounce(function(){
			$('#agency_user_update_form').submit(); 
		},500,true);
	...
	function agency_user_delete() {
		return confirm('<spring:message code="fap.err.common.user_delete" javaScriptEscape="true" />');
	}
	...
	function company_user_delete() {
		...
		var windowObj = window.open("/fap/agency/agency_user_info_delete_form", "회원 탈퇴 폼", settings);
	}
...
<div class="join-wrap">
	<div id="subcontents">
	<form action="/fap/agency/agency_user_update_form" method="post" enctype="multipart/form-data" id="agency_user_update_form">
		<input type="hidden" id="user_origin_email" name="user_origin_email" size="30" value="${agency_user.user_email}">
		<div class="joinBox">
			<div class="jointt">
				<p class="jointxt1">
			...
			<div class="join-contents">
				...
				<div class="joinBBS">
						<div class="joinBBS-row">
							...
							<div class="joinrt">
								<input type="text" id="user_id" name="user_id" value="${agency_user.user_id}" readonly="readonly">
							</div>
						</div>
						<div class="joinBBS-row">
						...
						<div class="joinBBS-row">
							...
							<div class="joinrt"><input type="text" id="user_nm" name="user_nm" value="${agency_user.user_nm}" placeholder="<spring:message code="fap.placholder.name" />"></div>
						</div>
						<div class="joinBBS-row">
							<div class="joinlt"><spring:message code="fap.common.email" /></div>
							<div class="joinrt">
								<input type="text" id="user_email" name="user_email" size="30" value="${agency_user.user_email}">
								<button class="join_btn" type="button" onclick="checkEmailIsDuplicated();">
									<spring:message code="fap.common.duplicate_ck" />
								</button>
							</div>
						</div>
						<input type="hidden" id="user_birth" name="user_birth" value="${agency_user.user_birth}" readonly="readonly"><br>
			...
			<div class="joinBox">
				<div class="join-policy">
					<div id="joinBtn">
						<button type="button" class="join-btn icon" onclick="form_check();"><spring:message code="fap.common.user_update"/></button>
						<!-- var="submitText" -->
					</div>
					<!-- 수정해야됨 -->
					<!-- 회원 탈퇴 -->
		...
		<!-- 회원 탈퇴 -->
		<%-- <form action="/fap/agency/agency_user_info_delete" method="post" onsubmit="return agency_user_delete();">
			<input type="hidden" name="user_id" value="${agency_user.user_id}">
		</form> --%>
	</div>
</div>

...
<title>Bridge Job Fair</title>
	...
	<script type="text/javascript">
		...
		function return_list() {
			location.href="/fap/user/user_job_advertisement_list_form";
		}
		// Q&A
		function qna_list(fap_job_ad_seq){
			location.href="/fap/user/user_job_qna_management?fap_job_ad_seq="+fap_job_ad_seq;
		}
	
		var myApp = angular.module('myapp', []);
		
		myApp.controller('readJobAdvertisement', ['$scope','$http','$compile', function($scope, $http,$compile) {
		
			//회사 로고 정보
			var company_info = '${company_info}';
			var company_info_json = JSON.parse(company_info);
			$('#imgDiv').append('<img style="width:30%; height:30%;" alt="" id="'+company_info_json.fap_comp_log_saved+'" src="/fap/company/user_logo_image/' + company_info_json.fap_comp_id + '/' + company_info_json.fap_comp_log_saved + '">');
			...
			// [start] 내가담은기업에 추가하기 시작
			$scope.insertMyFavoriteCompany = function() {
				var fap_job_ad_seq = $('#jobAdvertisementInfoForm').find('#fap_job_ad_seq').val();
				var fap_jobfair_divide_seq = $('#jobAdvertisementInfoForm').find('#fap_jobfair_divide_seq').val();
				var fap_jobfair_seq = $('#jobAdvertisementInfoForm').find('#fap_jobfair_seq').val();
				var jobAdvertisementInfoForm = {
						'fap_job_ad_seq' : fap_job_ad_seq
						,'fap_jobfair_divide_seq' : fap_jobfair_divide_seq
						,'fap_jobfair_seq' : fap_jobfair_seq
				}
				$http({
					url: 'insert_my_favorite_company',
					method: 'post',
					headers: {
						'Content-Type': 'application/json',
						"AJAX" : true
					},
					data: JSON.stringify(jobAdvertisementInfoForm),
					responseType : 'text'
				}).then(function successCallback(response) {
					if(response.data == 'insertDuplicate'){
						alert('이미 담기목록에 있는 기업입니다.');
					}else if(response.data == 'insertSuccess'){
						alert('담기목록에 추가하였습니다.');
					}else {
						alert('담기에 실패하였습니다. 관리자에게 문의하세요.');
					}
			...
			$scope.changeStatistics = function() {
				var selectedValue = $('#choicedStatistics option:selected').val();
				//기업규모 통계부분
				if(selectedValue == 'companyScale'){
					...
					$http({
						...
						function drawChart() {
						  var data = google.visualization.arrayToDataTable([
						    ['통계구분', '현재회사', '잡페어 평균'],
						    ['종업원 수', response.data.selected_employee_num, response.data.total_employee_num_avg],
						    ['자본금', response.data.selected_stock, response.data.total_stock_avg],
						    ['최근 3년 매출액', response.data.selected_3years_avg_sales, response.data.total_3years_avg_sales_avg]
						  ]);
						  var options = {
						    chart: {
						      title: '기업규모 통계'
						  ...
						  var chart = new google.charts.Bar(document.getElementById('resultStatistics'));
				...
				//전형 단계별 지원/합격률(준비중)
				else if(selectedValue == 'ProcessAcceptanceRate'){
					var imgTag = document.createElement('img');
					$('#resultStatistics').empty();
					$(imgTag).attr('src', '<c:url value="/resources/segroup/images/SorryBecauseWaiting.png" />');
					$('#resultStatistics').append(imgTag);
				}
				//통계선택부분
				else{
					$('#resultStatistics').empty();
			...
			//[start] 해당 기업 지원가능 여부 체크 시작
			$scope.applyThisJobAdvertisement = function() {
				var fap_jobfair_divide_seq = $('#applyThisJobAdvertisementForm').children('#fap_jobfair_divide_seq').val();
				var fap_jobfair_seq = $('#applyThisJobAdvertisementForm').children('#fap_jobfair_seq').val();
				var fap_job_ad_seq = $('#applyThisJobAdvertisementForm').children('#fap_job_ad_seq').val();
				var applyApplicableCheckInfo = {
	 			...
	 			$http({
					...
					switch(response.data){
						...
						default : 
							alert('지원페이지로 이동합니다.');
							$('#applyThisJobAdvertisementForm').submit();    
					}
				}, function errorCallback(response) {
					console.log(response);
					if(response.status == 403){
...
<body ng-app="myapp" ng-controller="readJobAdvertisement" id="company_job_ad_body">
	...
	<div class="job-ad-wrap">
			    	...
			    	<div class="job-ad-row">
						...
						<div class="job-ad-rt check-row">
							<span class="job-check long-check">
								...
								<input type="checkbox" name="jobCategoryList[0].fap_job_category_gb" value="C3700">
							</span>
							<span class="job-check long-check md-200">
								<label class="job-check-label">
			    	...
			    	<div class="job-ad-row">
						...
						<div class="job-ad-rt check-row">
							<span class="job-check long-check">
								...
								<input type="checkbox" name="jobCategoryList[5].fap_job_category_gb" value="C3705">
							</span>
							<span class="job-check long-check md-200">
								<label class="job-check-label">
								...
								<div class="sub-area-inner">
									<div class="bar-inner">
										...
										<span class="inner-item">
											<textarea disabled="disabled" rows="4" cols="20" name="jobCategoryList[4].JobCategoryEtc.fap_job_category_etc_dtl" maxlength="200" ng-model="::jobAdCategory.JobCategoryEtcList[0].fap_job_category_etc_dtl" placeholder="200<spring:message code="fap.placeholder.limit_characters" />"></textarea>
										</span>
									</div>
								</div>
							</div>
		...
		<!-- <div id="content-5">
		    	<h4>통계자료</h4>
		    	<select id="choicedStatistics" ng-model="choicedStatistics" ng-change="changeStatistics()">
		    		<option value="">---통계를 선택해주세요---</option>
		    		<option value="companyScale">기업규모</option>
		    		<option value="ProcessAcceptanceRate">전형 단계별 지원/합격률</option>
		    	</select>
		    	<div id="resultStatistics"></div>
		    </div> -->
		...
		<div class="job-ad-btn">
			...
			<!-- 해당 채용공고에  지원이 가능한지 여부 판단해서 가능하면 버튼을 보여준다-->
            <c:if test="${whetherJobAdApply eq 'possible'}">
            	<input type="button" class="side-btn btn-gradient red small" value="지원하기" ng-click="applyThisJobAdvertisement()">
            	<!-- 지원하기 -->
            	<form id="applyThisJobAdvertisementForm" action="apply_this_job_advertisement_form" method="post">
           	...
           	<!-- 기업담기 -->
           	<button type="button" class="side-btn btn-gradient blue small" ng-click="insertMyFavoriteCompany()">담아놓기</button>
           	<button type="button" class="side-btn btn-gradient orange small" onclick="qna_list(${fap_job_ad_seq})">질문하기</button>
           	<form id="jobAdvertisementInfoForm">
           		<input type="hidden" id="fap_job_ad_seq" name="fap_job_ad_seq" value="${fap_job_ad_seq}">
           		<input type="hidden" id="fap_jobfair_divide_seq" name="fap_jobfair_divide_seq" value="${fap_jobfair_divide_seq}">
           		<input type="hidden" id="fap_jobfair_seq" name="fap_jobfair_seq" value="${fap_jobfair_seq}">
           	</form>
			...
			<div id="side-bar" class="" style="">
				<ul class="section table-of-contents">
			      ...
			      <li><button class="side-btn btn-gradient cyan small" onclick="return_list()"><spring:message code="fap.jobfair.main.board.list" /></button></li>
			       <c:if test="${whetherJobAdApply eq 'possible'}">
            			<li><input type="button" class="side-btn btn-gradient red small" value="지원하기" ng-click="applyThisJobAdvertisement()"></li>
            		</c:if>
            	<li><button type="button" class="side-btn btn-gradient blue small" ng-click="insertMyFavoriteCompany()">담아놓기</button></li>
           		<li><button type="button" class="side-btn btn-gradient orange small" onclick="qna_list(${fap_job_ad_seq})">질문하기</button></li>
				</ul>
     		 </div>
		</div>
	...
	<%-- <table>
		<tr>
			<th>
				<img src="<c:url value="/resources/segroup/images/kr.png" />">
			</th>
    	<tr>
       		<th style="font-size: 10px;">기업명</th>
        	<td>${jobAdvertisement.fap_comp_ja_nm}</td>
    	</tr>
    	<tr>
        	<th style="font-size: 10px;">기업그룹</th>
            <td>${jobAdvertisement.fap_job_ad_groupcode}</td>
   		</tr>
    	<tr>
            <th style="font-size: 10px;">사업분야</th>
            <td>${jobAdvertisement.fap_job_ad_major_business}</td>
    	</tr>
    	<tr>
            <th style="font-size: 10px;">주소</th>
            <td>${jobAdvertisement.fap_job_ad_address}</td>
    	</tr>
    	<tr>
            <th style="font-size: 10px;">설립일</th>
            <td>${jobAdvertisement.fap_job_ad_est_dt}</td>
    	</tr>
    	<tr>
            <th style="font-size: 10px;">자본금/매출액</th>
            <td>${jobAdvertisement.fap_job_ad_stock}엔/${jobAdvertisement.fap_job_ad_3years_avg_sales}엔</td>
    	</tr>
    	<tr>
            <th style="font-size: 10px;">사원수</th>
            <td>${jobAdvertisement.fap_job_ad_employee_num}명</td>
    	</tr>
    	<tr>
            <th style="font-size: 10px;">홈페이지</th>
            <td>${jobAdvertisement.fap_job_ad_homepage}</td>
    	</tr>
    	<tr>
    	...
    	<tr>
            ...
            <td>  	
            <!-- 해당 채용공고에  지원이 가능한지 여부 판단해서 가능하면 버튼을 보여준다-->
            <c:if test="${jobAdvertisement.whetherJobAdApply eq 'possible'}">
            	<input type="button" value="지원하기" ng-click="applyThisJobAdvertisement()">
            	<!-- 지원하기 -->
            	<form id="applyThisJobAdvertisementForm" action="apply_this_job_advertisement_form" method="post">
            		<input type="hidden" id="fap_job_ad_seq" name="fap_job_ad_seq" value="${jobAdvertisement.fap_job_ad_seq}">
            		<input type="hidden" id="fap_jobfair_divide_seq" name="fap_jobfair_divide_seq" value="${jobAdvertisement.fap_jobfair_divide_seq}">
            		<input type="hidden" id="fap_jobfair_seq" name="fap_jobfair_seq" value="${jobAdvertisement.fap_jobfair_seq}">
            	</form>
			</c:if>	
            	<!-- 기업담기 -->
            	<input type="button" value="담아놓기" ng-click="insertMyFavoriteCompany()">
            	<form id="jobAdvertisementInfoForm">
            		<input type="hidden" id="fap_job_ad_seq" name="fap_job_ad_seq" value="${jobAdvertisement.fap_job_ad_seq}">
            		<input type="hidden" id="fap_jobfair_divide_seq" name="fap_jobfair_divide_seq" value="${jobAdvertisement.fap_jobfair_divide_seq}">
            		<input type="hidden" id="fap_jobfair_seq" name="fap_jobfair_seq" value="${jobAdvertisement.fap_jobfair_seq}">
            	...
            	<!-- Q&A -->
            	<a href="/fap/user/user_job_qna_management?fap_job_ad_seq=${jobAdvertisement.fap_job_ad_seq}"><button>Q&A</button></a>
            </td>
    	</tr>
	</table> --%>
	<!-- <div id="container">
	    <input id="tab-1" type="radio" name="tab-group" checked="checked" />
	    <label for="tab-1">고용정보</label>
	
	    <input id="tab-2" type="radio" name="tab-group" />
	    <label for="tab-2">회사 소개 자료</label>
	
	    <input id="tab-3" type="radio" name="tab-group" />
	    <label for="tab-3">회사 소개 동영상</label>
	    
	    <input id="tab-4" type="radio" name="tab-group" />
	    <label for="tab-4">회사 사진</label>
	    
	    <input id="tab-5" type="radio" name="tab-group" />
	    <label for="tab-5">통계 자료</label>
	    
	    <input id="tab-6" type="radio" name="tab-group" />
	    <label for="tab-6">Q&A</label>
		<div id="content">
			<div id="content-1">
				<h4>고용정보 탭</h4>
			</div>

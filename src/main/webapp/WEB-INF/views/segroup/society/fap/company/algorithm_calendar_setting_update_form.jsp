...
<title>Bridge Job Fair</title>
	...
	myApp.controller('CompanyController', ['$scope','$compile','$http', function($scope,$compile,$http){
		...
		$scope.selectCheck = function(){
			...
			var jobfair_st = $scope.result.jobfair_set.fap_jobfair_set_st;
			var jobfair_et = $scope.result.jobfair_set.fap_jobfair_set_et;
			var jobfair_room_st = $scope.result.jobfair_set.fap_jobfair_set_room_st;
			var jobfair_room_et = $scope.result.jobfair_set.fap_jobfair_set_room_et;
		    ...
		    var lastLength = $scope.result.job_plan_list.length - 1;
			var list = $scope.result.job_plan_list;
			...
			$.each(list , function(index, item){
				//마지막
				if(index == lastLength){
					context += '<td gubun="'+item.fap_la_job_plan_gb+'" day="'+item.fap_la_job_plan_dt+'" ng-class="{\'disabled\' : \''+item.fap_la_job_plan_gb+'\' == \'E0202\',\'ui-selected\' : \''+item.fap_la_job_plan_gb+'\' == \'E0201\'}"></td>';
					context += '</tr>';
				}else{
					//처음
					if(index == 0){
						context += '<tr class="day-list">';
						context += '<th time="'+item.fap_la_job_plan_tm+'">';
						context += item.fap_la_job_plan_tm+':00'+"~"+(parseInt(item.fap_la_job_plan_tm)+1)+':00';
						context += '</th>';
						context += '<td gubun="'+item.fap_la_job_plan_gb+'" day="'+item.fap_la_job_plan_dt+'" ng-class="{\'disabled\' : \''+item.fap_la_job_plan_gb+'\' == \'E0202\',\'ui-selected\' : \''+item.fap_la_job_plan_gb+'\' == \'E0201\'}"></td>';
					}
					//시간이 같다면
					else if(list[index].fap_la_job_plan_tm == list[index+1].fap_la_job_plan_tm){
						context += '<td gubun="'+item.fap_la_job_plan_gb+'" day="'+item.fap_la_job_plan_dt+'" ng-class="{\'disabled\' : \''+item.fap_la_job_plan_gb+'\' == \'E0202\',\'ui-selected\' : \''+item.fap_la_job_plan_gb+'\' == \'E0201\'}"></td>';
					}
					//시간이 바뀌면
					else{
						...
						context += '</tr>';
						context += '<tr class="day-list">';
						context += '<th time="'+list[index+1].fap_la_job_plan_tm+'">';
						context += list[index+1].fap_la_job_plan_tm+':00'+"~"+(parseInt(list[index+1].fap_la_job_plan_tm)+1)+':00';
						context += '</th>';
					}
				}
...
<body id="myBody" ng-app="myapp" ng-controller="CompanyController">
	<form action="algorithm_calendar_setting_update" method="post" onsubmit="return formCheck()">
		<input type="hidden" name="fap_job_ad_seq" id="fap_job_ad_seq" value="{{result.fap_job_ad_seq}}">
		<input type="hidden" name="fap_la_job_set_seq" value="{{result.fap_la_job_set_seq}}">
		
		<span>{{diff+1}}</span>일 중
		<select name="fap_la_job_set_join_dt" ng-model="result.fap_la_job_set_join_dt">
			<option ng-repeat="i in range(diff+1)" value="{{i + 1}}">{{i + 1}}</option>
		</select>
		일 참가가능 전형은
		<select name="fap_la_job_set_itv_dt" ng-model="result.fap_la_job_set_itv_dt">

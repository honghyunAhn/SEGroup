...
//flag 1:등록,2:임시저장
	...
	myApp.controller('JobAdController',['$scope','$compile', function($scope, $compile) {
		...
		for(var i in jobIdealPerson.fap_job_work){
			...
			context +='</div>';
			context +='<div class="job-ad-rt">';
			context +='<textarea rows="4" cols="20" name="jobWorkList[' + count + '].fap_job_work_dtl" maxlength="1000" placeholder="1000<spring:message code="fap.placeholder.limit_characters" />" ng-model="::jobIdealPerson.fap_job_work[' + i + '].fap_job_work_dtl"></textarea>';
			context +='</div>';
			context +='</div>';
			context +='</div>';
			context +='</div>';
		...
		for(var i in jobIdealPerson.fap_job_pref){			
			...
			context +='</div>';
			context +='<div class="job-ad-rt">';
			context +='<textarea rows="4" cols="20" name="jobPrefList['+count+'].fap_job_pref_dtl" maxlength="200" placeholder="200<spring:message code="fap.placeholder.limit_characters" />" ng-model="::jobIdealPerson.fap_job_pref[' + i + '].fap_job_pref_dtl"></textarea>';
			context +='</div>';
			context +='</div>';
			context +='</div>';
			context +='</div>';
		...
		for(var i in jobRecruitPcs){
			...
			context += '</div>';
			context += '<div class="job-ad-rt">';
			context += '<textarea rows="4" cols="20" name="jobRecruitPcsList['+count+'].fap_job_recruit_pcs_dtl" maxlength="400" placeholder="400<spring:message code="fap.placeholder.limit_characters" />" ng-model="::jobRecruitPcs['+i+'].fap_job_recruit_pcs_dtl"></textarea>';
			context += '</div>';
			context += '</div>';
			context += '</div>';			
			...
			if(jobRecruitPcs[i].fap_job_recruit_pcs_gb == 'C2501'){
				...
				if(jobRecruitPcs[i].job_exam.job_exam_file != null && jobRecruitPcs[i].job_exam.job_exam_file.fap_job_exam_file_origin != null){
					...
					context += '</a>'
					context += '<button class="btn-delete" type="button" onclick="update_job_exam_file(' + i + ')">Delete</button>';
				}else{
		...
		$scope.addRecruitExam = function(selected_obj) {
			...
			context += '</div>';
			context += '<div class="job-ad-rt">';
			context += '<input type="file" name="jobRecruitPcsList['+count+'].jobExam.jobExamFile.fap_job_exam_file">';
			context += '<label for="fap_job_exam_file"><spring:message code="fap.placeholder.file_size" /></label>';
			context += '</div>';
			context += '</div>';
			context += '</div>';
		...
		for(var i in jobPayList){
			if(jobPayList[i].fap_job_pay_gb == "C2201"){
				...
				context += '<h5 class="inner-title"><spring:message code="fap.jobad.extra_fee_name" /></h5>';
				context += '<span class="inner-item">';
				context += '<input type="text" class="half" name="jobPayList['+count+'].fap_job_pay_dtl" ng-model="::jobAdPay.JobPayList[' + i + '].fap_job_pay_dtl"  maxlength ="200"/>';
				context += '</span>';
				context += '</div>';
				context += '<div class="inner-float">';
				context += '<h5 class="inner-title right-title"><spring:message code="fap.jobad.extra_fee" /></h5>';
...
<script type="text/javascript">
	...
	//저장시 추가 파라미터를 넣어주는 함수(사용중)
	function save_job_ad() {
		
		if(form_check(1) == false) {
			return;
		}
		...
		if( $("#fap_job_ad_state").val() == null || $("#fap_job_ad_state").val() == undefined){
			...
			input.setAttribute("type", "hidden");
			input.setAttribute("name", "fap_job_ad_state");
			input.setAttribute("value", "C2100");
			input.setAttribute("id", "fap_job_ad_state");
...
<body ng-app="myapp" ng-controller="JobAdController" id="company_job_ad_body">
	...
	<div class="job-ad-wrap">
		...
		<form action="company_job_ad_update" method="post" enctype="multipart/form-data" id="insert_form">
		<div class="job-ad-box">
			...
			<div class="job-ad-contents scrollspy">
				<div class="job-ad-row">
					...
					<div class="job-ad-rt">
						<input class="input2" type="text" name="fap_job_ad_title" id="fap_job_ad_title" maxlength="50" placeholder="50<spring:message code="fap.placeholder.limit_characters" />" ng-model="::jobAdCategory.fap_job_ad_title">
					</div>
		    	</div>
		    	<div class="job-ad-row">
					<!-- 기업명(일본어) -->
				...
				<div class="job-ad-row">
					...
					<div class="job-ad-rt-la">
						<input class="input3 currency" id="fap_job_ad_stock" type="text" name="fap_job_ad_stock" ng-model="::jobAd.fap_job_ad_stock" blur-currency maxlength="22">
		    	...
		    	<div class="job-ad-row">
					...
					<div class="job-ad-rt">
						<input class="input3 currency" id="fap_job_ad_employee_num" type="text" name="fap_job_ad_employee_num" ng-model="::jobAd.fap_job_ad_employee_num" blur-currency maxlength="9">
		...
		<div id="hire_info" class="job-ad-box scrollspy">
			...
			<div class="job-ad-contents">
				...
				<div class="job-ad-row m50">
					...
					<div class="job-ad-rt">
						<input class="input2 currency" id="fap_job_ad_recruit_num" ng-model="::jobAdCategory.fap_job_ad_recruit_num" type="text" name="fap_job_ad_recruit_num" blur-currency maxlength="9">
						<span class="hiddenTx">名</span>
					</div>
		    	</div>	
		...
		<div class="job-ad-btn">
			<button type="button" class="side-btn btn-gradient blue small" onclick="save_job_ad();"><spring:message code="fap.jobad.register_jobad" /></button>
		...
		<div class="side-bar">
			<div id="side-bar" class="" style="">
				<ul class="section table-of-contents">
			      ...
			      <li><button class="side-btn btn-gradient blue small" onclick="save_job_ad();"><spring:message code="fap.jobad.register_jobad" /></button></li>

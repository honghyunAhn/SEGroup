...
$(function() {
	 $('input[name="fap_jobfair_seq"]').val(${fap_jobfair_seq});
	   $('input[name="fap_job_ad_seq"]').val(${fap_job_ad_seq});
	   $('input[name="fap_jobfair_title"]').val("${fap_jobfair_title}");
	   $('input[name="fap_jobfair_devide_gb"]').val("${fap_jobfair_devide_gb}");
...
<script type="text/javascript">
	$(document).ready( function() {
		...
		$(".score_print").on("click",function(){
		      var popupX = (window.screen.width / 2) - ( 1200 / 2 );
		      var popupY = (window.screen.height / 2) - ( 700 / 2 ) - 50;
		      
		      var score_popup = document.score_popup;
		      window.open('','score_print',"width=1200,height=700,left='+popupX+',top='+popupY+',screenX='+popupX+',screenY='+popupY+',toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=yes,scrollbars=no,copyhistory=no,scrollbars=yes");
		      score_popup.target = 'score_print';
		      score_popup.submit();
		   });
		...
		function itvLimitFnc(btnId){
	    	...
	    	if(ck_count >= 0){
	    		...
	    		$.ajax({
	    			url : '/fap/company/interview_limit_check',
	    			type : 'post',
	    			data : {
...
<div class="join-wrap">	
	<div id="subcontents">
		<c:if test="${final_select_info.fap_jobfair_divide_final_select_et != null}">
			<div class="finalDateDiv">
				<c:set var="final_select_st" value="${final_select_info.fap_jobfair_divide_final_select_st}"/>
				<c:set var="final_select_et" value="${final_select_info.fap_jobfair_divide_final_select_et}"/>
				<span style="color: red">${fn:substring(final_select_st,0,4) }<spring:message code="com.placeholder.year" />
				 ${fn:substring(final_select_st,5,7) }<spring:message code="com.placeholder.month" />
				  ${fn:substring(final_select_st,8,10) }<spring:message code="com.placeholder.day" /></span>
				~
				<span style="color: red">${fn:substring(final_select_et,0,4) }<spring:message code="com.placeholder.year" />
				 ${fn:substring(final_select_et,5,7) }<spring:message code="com.placeholder.month" />
				  ${fn:substring(final_select_et,8,10) }<spring:message code="com.placeholder.day" /></span>
		...
		<div id="peopleBBS" ng-if="applicantInfo[0] != null">
			<ul class="peoplechk" display="table">
			    ...
			    <c:if test="${fap_job_ad_state != 'C2102'}">
					<ul class="peoplefind">
						<li><button class="btn3 btn-primary" id="saveBtn"><spring:message code="fap.comp.app_rt_save" /></button></li>
						<li><button class="btn3 btn-primary score_print"><spring:message code="fap.common.score" /></button></li>
					</ul>
				</c:if>
				...
				<form action="/fap/company/update_fap_job_apply_result" method="POST" id="regular_apply_btn">
				...
				<input type="hidden" name="fap_jobfair_seq" value="${fap_jobfair_seq}">
            <input type="hidden" name="fap_jobfair_title" value="${fap_jobfair_title}">
            <input type="hidden" name="fap_jobfair_devide_gb" value="${fap_jobfair_devide_gb}">
					...
					<ul class="peoplemain" ng-repeat="finalCheck in info.finalResultMap" ng-if="::finalCheck.fap_job_app_final_rt_acp == 'D0000' ">
						...
						<li class="wid4">
							...
							<div class="screenshot" tooltip="/fap/user/resume_photo_image/{{info.resumeMap.fap_resume_pic_saved}}">
								...
								<img class="tooltip-image" alt="photo" src="<c:url value="/resources/segroup/society/fap/images/icon-photo.png" />"><br>
								&nbsp;<span ng-bind="info.resumeNmList[3].fap_resume_nm"></span>
							</div>
						...
						<li class="wid6 app_result" ng-repeat="result in info.applyResultMap">
		...
		<div id="peopleBBS" ng-if="missApplicantInfo[0] != null">
			<ul class="peoplechk" display="table">
				...
				<form action="/fap/company/update_missmatching_fap_job_apply_result" method="POST" id="missmatching_apply_btn">
					<input type="hidden" name="fap_job_ad_seq" value="${fap_job_ad_seq}">
					<input type="hidden" name="fap_comp_id" value="${fap_comp_id}">
					<input type="hidden" name="fap_jobfair_divide_seq" value="${fap_jobfair_divide_seq}">
				<div ng-repeat="missInfo in missApplicantInfo" ng-if="missInfo.fap_job_ad_is_mismatching == 'C4901' " ng-init="missInfoIndex = $index">
					...
					<li class="wid4">
						...
						<div class="screenshot" tooltip="/fap/user/resume_photo_image/{{missInfo.missResumeMap.fap_resume_pic_saved}}">
							...
							<img class="tooltip-image" alt="photo" src="<c:url value="/resources/segroup/society/fap/images/icon-photo.png" />"><br>
							&nbsp;<span ng-bind="missInfo.missResumeNmList[3].fap_resume_nm"></span>
						</div>
			...
			<ul class="peoplemain2" ng-repeat="un_info in unacceptedApplicantInfo" ng-init="un_infoIndex = $index" >
				...
				<li class="wid4">
					<div class="screenshot" tooltip="/fap/user/resume_photo_image/{{un_info.unacceptedResumeMap.fap_resume_pic_saved}}">
							...
							<img class="tooltip-image" alt="photo" src="<c:url value="/resources/segroup/society/fap/images/icon-photo.png" />"><br>
							&nbsp;<span ng-bind="un_info.unacceptedResumeNmList[3].fap_resume_nm"></span>
						</div>
...
<form action="/fap/company/applicant_score_print_form" method="post" name="score_popup" id="score_popup">
	<input type="hidden" name="fap_jobfair_seq">
   <input type="hidden" name="fap_job_ad_seq">
   <input type="hidden" name="fap_jobfair_title">
   <input type="hidden" name="fap_jobfair_devide_gb">
</form>

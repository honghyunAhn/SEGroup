...
$(function() {
	...
	$(".score_print").off("click").on("click",function(){
		var sb = new StringBuilder();
		sb.Append('<input type="hidden" name="fap_jobfair_seq" value="'+${fap_jobfair_seq}+'">');
		sb.Append('<input type="hidden" name="fap_job_ad_seq" value="'+${fap_job_ad_seq}+'">');
		sb.Append('<input type="hidden" name="jobfair_title" value="'+"${fap_jobfair_title}"+'">');
		sb.Append('<input type="hidden" name="comp_en_nm" value="'+"${fap_comp_en_nm}"+'">');
		sb.Append('<input type="hidden" name="jobfair_divide_gb" value="'+"${fap_jobfair_divide_gb}"+'">');
		$("#score_popup").html(sb.ToString());
	...
	myApp.controller('CompanyController',['$scope','$compile', '$http','$timeout', function($scope, $compile, $http,$timeout) {
		...
		//승인전 지원자 정보
		var approvalYetList = '${approvalYetList}';
		approvalYetList = approvalYetList.replace(/(?:\r\n|\r|\n)/g, '\\r\\n');
		var approvalInfo = JSON.parse(approvalYetList);
		$scope.approvalInfo = JSON.parse(approvalYetList);
	   ...
	   //합격사유 등록 MODAL
		$scope.pass_comment_insert_draw = function(index, infoSeq){
			...
			context += '</div>';
			context += '<div class="form-row">';
			context += '<textarea name="fap_job_app_pass_comment" class="form-control" maxlength="500" style="text-align: left; width:670px; height: 100px;" readonly="readonly"></textarea>';
			context += '</div>';
			context += '</div>';
			context += '</div>';								
			context += '<div class="modal-footer">';
			context += '<button type="button" class="btn btn-primary" data-dismiss="modal">닫기</button>';
			context += '</div>';							
			context += '</form>';
			context += '</div>';
			context += '</div>';
		...
		$scope.pass_comment_update_draw = function(index, infoSeq, comment){
			...
			context += '</div>';
			context += '<div class="form-row">';
			context += '<textarea name="fap_job_app_pass_comment" class="form-control" maxlength="500" style="text-align: left; width:670px; height: 100px;" readonly="readonly">'+comment+'</textarea>';
			context += '</div>';
			context += '</div>';
			context += '</div>';								
			context += '<div class="modal-footer">';

...
<script type="text/javascript">
	...
	function update_submit() {
		if (formCheck()) {
			var update_form = $("#apply_update_form").serialize();
			$.ajax({
				type : "POST",
				url : "/edu/apply/apply_update",
				data : update_form,
	...
	function formCheck() {
		...
		// 비상연락처(입력3칸)가 비어있지 않다면
		if (em_phone1.length != 0 || em_phone2.length != 0 || em_phone3.length != 0) {
			// 비상연락처 숫자만 입력체크
			if (!new RegExp("[0-9]{2,3}").test(em_phone1) 
				|| !new RegExp("[0-9]{3,4}").test(em_phone2) 
				|| !new RegExp("[0-9]{4}").test(em_phone3)) {
				alert('비상연락처 입력이 올바르지 않습니다');
				return false;
			}
		}
			...
			$.each($(this).find("input"), function(index, item) {
				if (!$(this).hasClass("nullable")) {
					if (index < 4 && item.value.length == 0) {
						alert("학력사항은 필수항목 입니다.");
						eduResult = false;
					}
					if (index == 2) {
						//                            alert(item.value);
						//                            alert($(this).closest("ul").find("select").val());
						if ($(this).closest("ul").find("select").val() == "B1000") {
							if (today < item.value.replace(/\-/g, '')) {
								alert('졸업년월 입력이 올바르지 않습니다');
								eduResult = false;
							}
...
<body ng-app="myapp" ng-controller="ApplyController">
	<form id="apply_update_form" enctype="multipart/form-data">
			...
			<div class="apply_form_wrap">
				...
				<div class="form_wrap">
					...
					<div class="form_photo">
						<div ng-switch="!!eduApply.app_photo_origin">
							<div ng-switch-when="true">
								<div class="form_photo_area" id="imgDiv">
									<input type="hidden" name="app_photo_origin" value="{{eduApply.app_photo_origin}}">
									<input type="hidden" name="app_photo_saved" value="{{eduApply.app_photo_saved}}">
									<img class="edu_photo_img" alt="" src="/edu/apply/photo/{{eduApply.app_id}}_{{eduApply.gisu_seq}}/{{eduApply.app_photo_saved}}">
									<div class="text-center">
										<a class="display-inline" ng-href="/file_download?origin={{eduApply.app_photo_origin}}&saved={{eduApply.app_photo_saved}}&path=/edu/apply/photo/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.app_photo_origin}}</a>
										<img class="x_icon" ng-click="eduApply.app_photo_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
									</div>
								</div>
							</div>
							<div ng-switch-default>
								...
								<input type="hidden" name="app_photo_saved" value="{{eduApply.app_photo_saved}}">
								<input type="file" name="app_photo_file" id="app_photo_file" onchange="angular.element(this).scope().file_change(this)" accept="image/*">
								<br />
							</div>
						</div>
					</div>
					...
					<ul class="form_basic">
						...
						<li class="form_text">
							...
							( 면제사유:
							<input type="text" class="in_small" name="app_mt_etc" value="{{eduApply.app_mt_etc}}" ng-class="{'disabled' : eduApply.app_mt_ck!='B0602'}" maxlength="30" placeholder="30자 이내로 서술">
							)
						</li>
					</ul>
					<ul class="form_basic form_large">
						...
						<li class="form_large_text">
							<ul>
								<li class="form_text border_bt">
									<b>(우편번호)</b>
									<input type="text" class="in_mid" id="zipcode" name="app_zipcode" readonly="readonly" value="{{eduApply.app_zipcode }}" />
									<input type="button" class="in_small" id="btn" onclick="findAddr()" value="주소검색">
								</li>
								<li class="form_text border_bt">
									<input type="text" id="address" name="app_address" readonly="readonly" value="{{eduApply.app_address }}" />
								</li>
								<li class="form_text">
									<input type="text" id="addressDetail" name="app_address_detail" readonly="readonly" value="{{eduApply.app_address_detail }}" />
								</li>
							</ul>
						</li>
					</ul>
					...
					<ul class="form_basic">
						...
						<li class="form_text">
							<input type="text" class="in_small text-center" id="app_em_phone1" value="${fn:substring(em_phone,0,3) }" maxlength="3">
							-
							<input type="text" class="in_small text-center" id="app_em_phone2" value="${fn:substring(em_phone,3,7) }" maxlength="4">
							-
							<input type="text" class="in_small text-center" id="app_em_phone3" value="${fn:substring(em_phone,7,11) }" maxlength="4">
						</li>
					</ul>
					<ul class="form_basic">
						<li class="form_title">
					...
					<ul class="form_full_text edu_ul eduHistory" ng-repeat="eduHistory in eduApply.eduHistoryList">
						<li>
							<input type="text" class="text-center" name="eduHistoryList[{{$index}}].app_edu_sc_nm" value="{{eduHistory.app_edu_sc_nm}}" maxlength="60">
						</li>
						<li>
							<input type="text" class="text-center" name="eduHistoryList[{{$index}}].app_edu_major" value="{{eduHistory.app_edu_major}}" maxlength="60">
						</li>
						<li>
							<select class="select_gradu" name="eduHistoryList[{{$index}}].app_edu_gd_ck" ng-model="eduHistory.app_edu_gd_ck">
						...
						<li>
							<input type="text" class="text-center" name="eduHistoryList[{{$index}}].app_edu_gd_dt" id="app_eduHIstory_dt_{{$index}}" readonly="readonly" value="{{eduHistory.app_edu_gd_dt}}" />
						</li>
						<li class="li_last">
							<input type="text" class="text-center" name="eduHistoryList[{{$index}}].app_edu_sc_lo" value="{{eduHistory.app_edu_sc_lo}}" maxlength="30">
						</li>
						<div class="clear"></div>
						<ul class="require gradu_con" ng-class="{'display-block' : eduHistory.app_edu_gd_ck=='B1001'}">
							...
							<li class="require_add li_last" style="width: 60%;">
								<input type="text" class="text-center nullable" name="eduHistoryList[{{$index}}].app_edu_gd_rq" value="{{eduHistory.app_edu_gd_rq}}">
							</li>
						</ul>
					</ul>
					<h2>
					...
					<div ng-switch="!!eduApply.careerList.length">
						<div ng-switch-when="true">
							<ul class="form_full_text form_full_6 car_ul career" ng-repeat="career in eduApply.careerList">
								<li>
									<input type="text" class="text-center" name="careerList[{{$index}}].app_crr_place" value="{{career.app_crr_place}}" maxlength="30">
								</li>
								<li>
									<input type="text" class="text-center" name="careerList[{{$index}}].app_crr_position" value="{{career.app_crr_position}}" maxlength="10">
								</li>
								<li>
									<input type="text" class="text-center" name="careerList[{{$index}}].app_crr_st" id="app_crr_frm_{{$index}}" readonly="readonly" value="{{career.app_crr_st}}">
								</li>
								<li>
									<input type="text" class="text-center" name="careerList[{{$index}}].app_crr_et" id="app_crr_end_{{$index}}" readonly="readonly" value="{{career.app_crr_et}}">
								</li>
								<li>
									<input type="text" class="text-center" name="careerList[{{$index}}].app_crr_dept" value="{{career.app_crr_dept}}" maxlength="30">
								</li>
								<li class="li_last">
									<input type="text" class="text-center nullable" name="careerList[{{$index}}].app_crr_phone" value="{{career.app_crr_phone}}" maxlength="13">
								</li>
								<div class="clear"></div>
							</ul>
						</div>
					...
					<div ng-switch="!!eduApply.studyList.length">
						<div ng-switch-when="true">
							<ul class="form_full_text form_full_6 edu2_ul study" ng-repeat="study in eduApply.studyList">
								<li>
									<input type="text" class="text-center" name="studyList[{{$index}}].app_study_nm" value="{{study.app_study_nm}}" maxlength="30">
								</li>
								<li>
									<input type="text" class="text-center" name="studyList[{{$index}}].app_study_st" id="app_study_frm_{{$index}}" readonly="readonly" value="{{study.app_study_st}}">
								</li>
								<li>
									<input type="text" class="text-center" name="studyList[{{$index}}].app_study_et" id="app_study_end_{{$index}}" readonly="readonly" value="{{study.app_study_et}}">
								</li>
								<li>
									<input type="text" class="text-center" name="studyList[{{$index}}].app_study_ag" value="{{study.app_study_ag}}" maxlength="30">
								</li>
								<li class="li_last" style="width: 33.3%;">
									<input type="text" class="text-center" name="studyList[{{$index}}].app_study_detail" value="{{study.app_study_detail}}" maxlength="100">
								</li>
								<div class="clear"></div>
							</ul>
						</div>
					...
					<div ng-switch="!!eduApply.languageList.length">
						<div ng-switch-when="true">
							<ul class="form_full_text lang_ul language" ng-repeat="language in eduApply.languageList">
								<li>
									<input type="text" class="text-center nullable" name="languageList[{{$index}}].app_lang_nm" value="{{language.app_lang_nm}}" maxlength="10">
								</li>
								<li>
									<input type="text" class="text-center" name="languageList[{{$index}}].app_lang_test_nm" value="{{language.app_lang_test_nm}}" maxlength="30">
								</li>
								<li>
									<input type="text" class="text-center" name="languageList[{{$index}}].app_lang_grade" value="{{language.app_lang_grade}}" maxlength="10">
								</li>
								<li>
									<input type="text" class="text-center nullable" name="languageList[{{$index}}].app_lang_ag" value="{{language.app_lang_ag}}" maxlength="30">
								</li>
								<li class="li_last">
									<input type="text" class="text-center" name="languageList[{{$index}}].app_lang_obtain_dt" id="app_lang_dt_{{$index}}" readonly="readonly" value="{{language.app_lang_obtain_dt}}">
								</li>
							</ul>
						</div>
						<div ng-switch-default>
					...
					<div ng-switch="!!eduApply.licenseList.length">
						<div ng-switch-when="true">
							<ul class="form_full_text form_full_4 lic_ul license" ng-repeat="license in eduApply.licenseList">
								<li>
									<input type="text" class="text-center" name="licenseList[{{$index}}].app_license_nm" value="{{license.app_license_nm}}" maxlength="30">
								</li>
								<li>
									<input type="text" class="text-center" name="licenseList[{{$index}}].app_license_grade" value="{{license.app_license_grade}}" maxlength="10">
								</li>
								<li>
									<input type="text" name="licenseList[{{$index}}].app_license_ag" class="text-center nullable" value="{{license.app_license_ag}}" maxlength="30">
								</li>
								<li class="li_last">
									<input type="text" class="text-center" name="licenseList[{{$index}}].app_license_obtain_dt" id="app_license_dt_{{$index}}" readonly="readonly" value="{{license.app_license_obtain_dt}}">
								</li>
							</ul>
						</div>
						<div ng-switch-default>
					...
					<ul class="form_full_text">
						<li style="width: 40%;">
							<input type="radio" class="apply_check" name="app_take_ck" value="B0700" ng-model="eduApply.app_take_ck">
							<code value="B0700"></code>
							<input class="apply_none" type="radio" name="app_take_ck" value="B0701" ng-model="eduApply.app_take_ck">
							<code value="B0701"></code>
						</li>
						<li>
							<input type="text" name="app_take_dt" id="app_take_dt" readonly="readonly" class="apply_inp text-center" ng-class="{'disabled' : eduApply.app_take_ck=='B0701'}" value="{{eduApply.app_take_dt}}">
						</li>
						<li class="li_last" style="width: 40%;">
							<input type="text" class="apply_inp text-center" name="app_take_rt" ng-class="{'disabled' : eduApply.app_take_ck=='B0701'}" value="{{eduApply.app_take_rt}}" maxlength="10">
						</li>
					</ul>
					<h2>
						<img class="form_arrow" src="<c:url value="/resources/segroup/society/edu/image/main/ico_black.png" />" alt="" />
					...
					<ul class="form_full_text">
						<li style="width: 40%;">
							<input type="radio" class="k_move_check" name="app_kmove_ck" value="B0900" ng-model="eduApply.app_kmove_ck">
							<code value="B0900"></code>
							<input class="k_move_none" type="radio" name="app_kmove_ck" value="B0901" ng-model="eduApply.app_kmove_ck">
							<code value="B0901"></code>
						</li>
						<li style="width: 15%;">
							<input type="text" class="k_move_inp text-center" name="app_kmove_nm" ng-class="{'disabled' : eduApply.app_kmove_ck=='B0901'}" value="{{eduApply.app_kmove_nm}}" maxlength="15">
						</li>
						<li style="width: 15%;">
							<input type="text" class="k_move_inp text-center" name="app_kmove_ag" ng-class="{'disabled' : eduApply.app_kmove_ck=='B0901'}" value="{{eduApply.app_kmove_ag}}" maxlength="15">
						</li>
						<li style="width: 15%;">
							<input type="text" name="app_kmove_st" id="app_kmove_frm" readonly="readonly" class="k_move_inp text-center" ng-class="{'disabled' : eduApply.app_kmove_ck=='B0901'}" value="{{eduApply.app_kmove_st}}">
						</li>
						<li class="li_last" style="width: 15%;">
							<input type="text" name="app_kmove_et" id="app_kmove_end" readonly="readonly" class="k_move_inp text-center" ng-class="{'disabled' : eduApply.app_kmove_ck=='B0901'}" value="{{eduApply.app_kmove_et}}">
						</li>
					</ul>
					<h2>
						<img class="form_arrow" src="<c:url value="/resources/segroup/society/edu/image/main/ico_black.png" />" alt="" />
					...
					<ul class="form_full">
						...
						<li class="li_last margin_8" style="width: 50%;">
							<input type="radio" class="li_check" name="app_abroad_ck" value="B0800" ng-model="eduApply.app_abroad_ck">
							<code value="B0800"></code>
							<input type="radio" class="li_none" name="app_abroad_ck" value="B0801" ng-model="eduApply.app_abroad_ck">
					...
					<div ng-switch="!!eduApply.overseasList.length">
						<div ng-switch-when="true">
							<ul class="form_full_text fore_ul overseas" ng-repeat="overseas in eduApply.overseasList">
								<li style="width: 25%;">
									<input type="text" class="li_inp text-center" name="overseasList[{{$index}}].app_overseas_nm" ng-class="{'disabled' : eduApply.app_abroad_ck=='B0801'}" value="{{overseas.app_overseas_nm}}" maxlength="30">
								</li>
								<li style="width: 25%;">
									<input type="text" class="li_inp text-center" name="overseasList[{{$index}}].app_overseas_st" id="app_overseas_frm_{{$index}}" readonly="readonly" ng-class="{'disabled' : eduApply.app_abroad_ck=='B0801'}" value="{{overseas.app_overseas_st}}">
								</li>
								<li style="width: 25%;">
									<input type="text" class="li_inp text-center" name="overseasList[{{$index}}].app_overseas_et" id="app_overseas_end_{{$index}}" readonly="readonly" ng-class="{'disabled' : eduApply.app_abroad_ck=='B0801'}" value="{{overseas.app_overseas_et}}">
								</li>
								<li class="li_last" style="width: 25%;">
									<input type="text" class="li_inp text-center" name="overseasList[{{$index}}].app_overseas_purpose" ng-class="{'disabled' : eduApply.app_abroad_ck=='B0801'}" value="{{overseas.app_overseas_purpose}}" maxlength="60">
								</li>
							</ul>
						</div>
						<div ng-switch-default>
					...
					<ul class="form_full_text">
						<li class="li_last" style="width: 100%; height: 240px;">
							<textarea name="app_self_intro" id="selfIntro">${app_self_intro}</textarea>
						</li>
					</ul>
					<h2>
						<img class="form_arrow" src="<c:url value="/resources/segroup/society/edu/image/main/ico_black.png" />" alt="" />
					...
					<ul class="form_full">
						<li class="sub_title li_last pop_2" style="width: 100%;">
							...
							<div class="form_popup file_pop">
								<img class="form_close" src="<c:url value="/resources/segroup/society/edu/image/sub/form_close.png" />" alt="" />
								<p>
									파일명 : 항목_이름 ex) 최종학력증명서_홍길동, 고용보험상실확인_홍길동<br> <br> 1) 최종학력증명서(필수)<br> - 졸업증명서, 학위증명서, 졸업장, 졸업예정증명서(졸업예정일 기재) <br> - (졸예자, 유예자 중) 상기의 서류가 제출 불가능할 경우(수료증명서 포함) <br> : 학적상태를 증명할 수 있는 서류 제출(졸업요건, 달성상태기재, 학교 직인필수)<br> <br> 2) 고용보험 상실내역 확인서(필수)<br> - 발급처: 고용보험(www.ei.go.kr) → 로그인 → 개인서비스<br> → 고용보험 가입(상용이력) 이력조회 <br> - 고용보험가입이력이 아예 없는 경우에도 해당화면 캡처하여 제출<br> ※1. 본인의 이름과 주민번호 앞 6자리, “고용보험조회 내역이 없습니다”라는 <br> 문구가 명확히 보이도록 캡쳐해주시기 바랍니다.<br> ※2. 고용보험 상실신고가 지연되어 상실내역 확인서 제출이 어려운 경우 : <br> 퇴사일이 명기 된 “퇴사증명서”를 제출 하고, 반드시 연수과정 개시 전까지 <br> 상실처리가 되어야 함(고용보험 상실일 역시 연수과정 개시이전으로 신고) <br> <br> 3) 출입국 사실증명서(필수)<br> - 출입국 조회기간: 개강일 기준 1년 전 일자 ~ 현재까지<br> - 발급처: 민원 24(www.minwon.go.kr) 로그인 → 출입국 기록 신청 <br> → 관련정보 입력 후 민원신청하기<br> ※1. 출입국 내역이 없어 온라인 조회가 안 되는 경우<br> : 동사무소에서 출입국 사실 증명원 발급<br> ※2. 조회기간 내 해외체류기간이 8개월 이상 되는 경우<br> : 방문국가를 확인할 수 있는 별도 증명서류 제출(여권 내 출입국 날인 사본 등)<br> <br>
									4) 워크넷 직무평가 결과(필수)<br> - 발급처: 워크넷(www.work.go.kr) → 로그인 →직업심리검사(성인)<br> → IT직무 기본역량검사를 실시 → 결과조회 → 파일로 저장후 첨부<br> <br> 5) 기타(선택)<br> - 일본어 공인자격(JLPT 또는 JPT 등)<br> - 정보처리기사 또는 산업기사 자격증 사본<br> - 개인의 경력/교육이수 내용을 확인할 수 있는 자료(포트폴리오 등)<br> - 그 외(IT관련 자격증 등)
								</p>
							</div>
						</li>
					</ul>
					<ul class="form_full_text" ng-switch="!!eduApply.app_edu_file_origin">
						<li class="li_last" style="width: 100%;" ng-switch-when="true">
							<input type="hidden" name="app_edu_file_origin" value="{{eduApply.app_edu_file_origin}}">
							<input type="hidden" name="app_edu_file_saved" value="{{eduApply.app_edu_file_saved}}">
							<div class="text-center">
								<a class="display-inline" ng-href="/file_download?origin={{eduApply.app_edu_file_origin}}&saved={{eduApply.app_edu_file_saved}}&path=/edu/apply/edu_history/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.app_edu_file_origin}}</a>
								<img class="x_icon" ng-click="eduApply.app_edu_file_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
							</div>
						</li>
					...
					<ul class="form_full_text" ng-switch="!!eduApply.app_isr_file_origin">
						<li class="li_last" style="width: 100%;" ng-switch-when="true">
							<input type="hidden" name="app_isr_file_origin" value="{{eduApply.app_isr_file_origin}}">
							<input type="hidden" name="app_isr_file_saved" value="{{eduApply.app_isr_file_saved}}">
							<div class="text-center">
								<a class="display-inline" ng-href="/file_download?origin={{eduApply.app_isr_file_origin}}&saved={{eduApply.app_isr_file_saved}}&path=/edu/apply/insurance/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.app_isr_file_origin}}</a>
								<img class="x_icon" ng-click="eduApply.app_isr_file_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
							</div>
						</li>
						<li class="li_last" style="width: 100%;" ng-switch-default>
							<input type="hidden" name="app_isr_file_saved" value="{{eduApply.app_isr_file_saved}}">
							<input type="file" class="app_files" name="app_isr_file" accept="image/*, application/pdf">
							<br />
						</li>
					</ul>
					<ul class="form_full" style="border-top: 0;">
					...
					<ul class="form_full_text" ng-switch="!!eduApply.app_imm_file_origin">
						<li class="li_last" style="width: 100%;" ng-switch-when="true">
							<input type="hidden" name="app_imm_file_origin" value="{{eduApply.app_imm_file_origin}}">
							<input type="hidden" name="app_imm_file_saved" value="{{eduApply.app_imm_file_saved}}">
							<div class="text-center">
								<a class="display-inline" ng-href="/file_download?origin={{eduApply.app_imm_file_origin}}&saved={{eduApply.app_imm_file_saved}}&path=/edu/apply/immigration/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.app_imm_file_origin}}</a>
								<img class="x_icon" ng-click="eduApply.app_imm_file_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
							</div>
						</li>
						<li class="li_last" style="width: 100%;" ng-switch-default>
							<input type="hidden" name="app_imm_file_saved" value="{{eduApply.app_imm_file_saved}}">
					...
					<ul class="form_full_text" ng-switch="!!eduApply.app_imm_file_origin">
						<li class="li_last" style="width: 100%;" ng-switch-when="true">
							<input type="hidden" name="app_worknet_file_origin" value="{{eduApply.app_worknet_file_origin}}">
							<input type="hidden" name="app_worknet_file_saved" value="{{eduApply.app_worknet_file_saved}}">
							<div class="text-center">
								<a class="display-inline" ng-href="/file_download?origin={{eduApply.app_worknet_file_origin}}&saved={{eduApply.app_worknet_file_saved}}&path=/edu/apply/worknet/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.app_worknet_file_origin}}</a>
								<img class="x_icon" ng-click="eduApply.app_worknet_file_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
							</div>
						</li>
						<li class="li_last" style="width: 100%;" ng-switch-default>
							<input type="hidden" name="app_imm_file_saved" value="{{eduApply.app_worknet_file_saved}}">
					...
					<div ng-repeat="i in [1,2,3]">
						...
						<ul class="form_full_text" ng-switch="!!eduApply.fileList[$index].app_file_origin">
							<li class="li_last" style="width: 100%;" ng-switch-when="true">
								<input type="hidden" name="fileList[{{$index}}].app_file_origin" value="{{eduApply.fileList[$index].app_file_origin}}">
								<input type="hidden" name="fileList[{{$index}}].app_file_saved" value="{{eduApply.fileList[$index].app_file_saved}}">
								<div class="text-center">
									<a class="display-inline" ng-href="/file_download?origin={{eduApply.fileList[$index].app_file_origin}}&saved={{eduApply.fileList[$index].app_file_saved}}&path=/edu/apply/etc/{{eduApply.app_id}}_{{eduApply.gisu_seq}}">{{eduApply.fileList[$index].app_file_origin}}</a>
									<img class="x_icon" ng-click="eduApply.fileList[$index].app_file_origin=false" src="<c:url value="/resources/segroup/society/edu/image/sub/x_icon.png" />" alt="" />
								</div>
							</li>
							<li class="li_last" style="width: 100%;" ng-switch-default>
								<input type="hidden" name="fileList[{{$index}}].app_file_saved" value="{{eduApply.fileList[$index].app_file_saved}}">
								<input type="file" class="app_files" name="fileList[{{$index}}].app_file" accept="image/*, application/pdf">
								<br />
							</li>
						</ul>
					</div>
				...
				<input type="button" onClick="update_submit();" class="apply_btn" value="수정하기">
			</div>
		</div>
	</form>
</body>

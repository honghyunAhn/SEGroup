...
<script type="text/javascript">
	...
	$(function() {
		...
		//처음 노출여부 불러오는 부분
		var edu_ban_eps = '${result.edu_ban_eps}';
		$('#' + edu_ban_eps).attr('checked', true);

		minMax("edu_ban_st", "edu_ban_et");
 		
		$("#curriculum").val("${result.edu_ban_nm}").attr("selected", "selected");
		$("#link").val("${result.edu_ban_link}").attr("selected", "selected");
	...
	function formCheck() {
		var edu_ban_nm = $("#edu_ban_nm");
		var edu_ban_link = $("#edu_ban_link");
		var edu_ban_file = $("#edu_ban_file")[0].files;
...
<body ng-app="myapp" ng-controller="SocietyEduAdminViewController">
	...
	<div class="custom_div">
		<form action="/edu/admin/banner_update"
			...
			<table class="gisu_crc_table">
				...
				<tbody>
					<tr>
						...
						<td>
							숨김<input type="radio" id="b2401" name="edu_ban_eps" value="b2401" checked="checked">
							노출<input type="radio" id="b2400" name="edu_ban_eps" value="b2400">
						</td> -->	
					</tr>
					<tr>
						<td class="name">과정명
						...
						<td>
							<input type="text" value="${result.edu_ban_nm}" name="edu_ban_nm" id="edu_ban_nm"maxlength="100" readonly="readonly">
						</td>
					</tr>
					<tr>
						<td class="name">링크
						<select id="link">
							<option id="seoul" selected="selected" value="https://www.softsociety.net/edu/apply/scitmaster_seoul">SC IT마스터(서울)</option>
						...
						<td><input type="text" name="edu_ban_link" id="edu_ban_link" value="${result.edu_ban_link}" readonly="readonly"></td>
					</tr>
					<tr>
						<td class="name">저장 된 이미지 파일</td>
						<td><input type="text" id="saved_img" name="saved_img" value="${result.edu_ban_origin}" readonly="readonly"></td>
					</tr>
					 <tr>
                    <td class="name">배너 등록 이미지 변경하기</td>
                    <td>
                        ...
                        <img alt="배너 등록 이미지 미리보기" src="${result.img_full_path}" id="avatar_info_image1" style="border: 1px solid black"/><br/>
                    </td>
                	</tr>
			</table>
			
			<input type="hidden" id="edu_ban_seq" name="edu_ban_seq" value="${result.edu_ban_seq}">
			<input type="hidden" id="edu_ban_saved" name="edu_ban_saved" value="${result.edu_ban_saved}">
			<input class="banner_submit" type="submit" value="수정하기">&nbsp;
			<a href="/edu/admin/banner_delete?edu_ban_seq=${result.edu_ban_seq}&edu_ban_saved=${result.edu_ban_saved}">
				<input class="banner_submit" type="button" value="삭제하기">
			</a>
		</form>
	</div>

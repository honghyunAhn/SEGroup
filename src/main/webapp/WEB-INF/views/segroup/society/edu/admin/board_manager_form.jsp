...
$(function() {
	$("#pageSelect").change(function(){
		...
		location.href="${path}/edu/admin/board_manager_form?pageCount="+pageCount+"&curPage=${map.adminPager.curPage}&search_division=${map.search_division}&search_type=${map.search_type}";
	});
})
...
function ilist(page){
    ...
    location.href="${path}/edu/admin/board_manager_form?&pageCount=${map.pageCount}&curPage="+page+"&search_division=${map.search_division}&search_type=${map.search_type}";
}

function board_content_commit(seq, nm){
	$("#board_seq").val(seq); 
	$("#board_nm").val(nm);
	$("#boardForm").submit();
}
...
function search(){
	var board_gb = $("#boardGubunSelect option:selected").val();
	var board_tp = $("#boardTypeSelect option:selected").val();
	
	$("#board_gb").val(board_gb);
	$("#board_tp").val(board_tp);
	
	//document.location.hash = board_gb + "," + board_tp;
	$("#search_type").val(board_tp);
	$("#search_division").val(board_gb);
	if(board_gb == 0){
		alert("게시판 구분을 선택해 주세요.");
		//$("#boardDiv").html("");
	}else if(board_tp == 0){
		alert("게시판 타입을 선택해 주세요.");
		//$("#boardDiv").html("");
	}else{
		location.href="${path}/edu/admin/board_manager_form?search_division="+board_gb+"&search_type="+board_tp;
	}
...
<div class="page_title"><h2>게시판 관리 페이지</h2></div>
		...
		<tbody>
			...
			<input type="hidden" class="boardSeq" value="${borad_seq}" />
				<tr>
					...
					<td><a onclick="board_content_commit('${data.board_seq}', '${data.board_nm}')">${data.board_nm}</a></td>
				</tr>
			</c:forEach>		
		</tbody>
	</table>
...
<form action="/edu/admin/board_contents_search" method="post" id="boardForm">

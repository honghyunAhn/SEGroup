...
<meta name="description" content="User Detailed BookInfo Page" />
	...
	$(function() {
		...
		$('#searchTextField').autocomplete({
			...
			select : function(event, selected) {
				$.each(bookInfos, function(index, book) {
					var selectedInfo = selected.item.value.split("|");
					if (book.BOOKINFO_BOOK_NM == selectedInfo[0] && book.BOOKINFO_AUTHOR == selectedInfo[1] 
						&& book.BOOKINFO_PUBLISHER == selectedInfo[2]){
						location.href = "/book/user/detailed_bookinfo?isbn='" + book.BOOKINFO_ISBN + "'";
					}
				});
			}
		});
		//책 세부정보
		$("#book_detail_img").html('<img onload="resizeImg(this, 250, 180);" alt="" src="/book/utility/download_bookinfo_image?origin='+selected_book.BOARD_FILE_ORIGIN+'&saved='+selected_book.BOARD_FILE_SAVED+'">');
		$("#book_detail_info").html('<h2>'+selected_book.BOOKINFO_BOOK_NM+'</h2>');
	});
	
	function advanced_submit(){
		var title = $("#advanced_title").val();
		var author = $("#advanced_author").val();
		var publisher = $("#advanced_publisher").val();
		var publish_start = $("#advanced_start_date").val();
		var publish_end = $("#advanced_end_date").val();
		var isbn = $("#advanced_isbn").val();
		var category = $("advanced_category:selected").val();
		
		$.ajax({
			type : "get",
			url : "/book/user/search_result",
			data : {
				bookinfo_book_nm : title,
				bookinfo_author : author,
				bookinfo_publisher : publisher,
				publish_start : publish_start,
				publish_end : publish_end,
				bookinfo_isbn : isbn,
				bookinfo_category : category
			},
			error : function(e) {
				console.log(e);
			}
...
<body>
	...
	<div style="padding-left:180px;padding-right:180px;margin:50px;">
		<div class="row fh5co-post-entry">
			<span id="book_detail_img" style="position:relative;width:20%;"></span>
			<span id="book_detail_info" style="position:relative;width:50%;"></span>
		</div>
	</div>

	<!-- <footer id="fh5co-footer">
		<p><small>&copy; All Rights Reserverd. <br> Designed by <a href="#" target="_blank">FREEHTML5.co</a>  Demo Images: <a href="http://unsplash.com/" target="_blank">Unsplash</a></small></p>
	</footer> -->

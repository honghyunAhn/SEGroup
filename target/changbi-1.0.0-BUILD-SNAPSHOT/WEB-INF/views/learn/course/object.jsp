...
function searchList(target_types) {
	if(typeof(target_types) == 'undefined') {
		target_types = $('#moveForm input[name="target_types"]').val();
	}
	var sort = $('#courseSort button.on').data('sort') || '';
	location.href = '<c:url value="/learn/course/object"/>?target_types='+target_types+'&sort='+sort;
}
</script>
		...
		<div class="body">
			<div class="sub-header">
				<p class="sub-nav">
					...
					<a href="#">연수 신청</a>
					<a class="nth-last" href='${contextPath}'>대상별 연수</a>
				</p>
				<h2 class="sub-title">대상별 연수 </h2>
				<!--p class="sub-title-copy">제목을 보조해주는 문구 삽입</p-->
			</div><!--//.sub-header-->
			<div class="sub-body">
				<c:if test="${not currentDevice.mobile }"><t:insertDefinition name="commonslide"/></c:if>
				<div class="tab-group">
					<ul class="tab-header sub x5">
						<li><a class="nth-1st <c:if test="${'0' eq param.get('target_types')}">on</c:if>" href="javascript:searchList(0)">전체 보기</a></li>
						<li><a class="nth-1st <c:if test="${'1' eq param.get('target_types')}">on</c:if>" href="javascript:searchList(1)">유아</a></li>
						<li><a class="nth-1st <c:if test="${'2' eq param.get('target_types')}">on</c:if>" href="javascript:searchList(2)">초등</a></li>
						<li><a class="nth-1st <c:if test="${'3' eq param.get('target_types')}">on</c:if>" href="javascript:searchList(3)">중등</a></li>
						<li><a class="nth-1st <c:if test="${'4' eq param.get('target_types')}">on</c:if>" href="javascript:searchList(4)">전문직</a></li>
					</ul>
					<div class="tab-body">
						<div class="list-top">
...
<!--// 1.3. 본문 -->
<form id="moveForm">
<input type="hidden" name="target_types" value="${param.target_types }"/>
</form>
...
function searchList(course_code) {
	...
	location.href = '<c:url value="/learn/course/course"/>?course_code='+course_code+'&sort='+sort;
}
</script>
						...
						<table id="list-learn" class="list">
							...
							<c:forEach var="list" items="${list}" varStatus="status">
								<tr <c:if test="${status.index == 0 }">class="nth-1st"</c:if>>
									<td class="item-title"><a href="<c:url value="/learn/course/detail"/>?course_id=${list.course_id}"><strong>${list.name}</strong></a>
										<span class="iconSet">
									...
									<td class="item-book">
										...
										<c:if test="${not empty list.main_tb_name }">${list.main_tb_name }</c:if>

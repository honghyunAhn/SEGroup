...
var itemList;
function orderBook() {
	var learn_app_id = $("input:radio[name='chkBook']:checked").val();
	if(!learn_app_id) {
		alert("연수 과정을 선택해 주십시오.");
		return;
	}

	var course_id = $("input:radio[name='chkBook']:checked").attr("data-course");
	var cardinal_id = $("input:radio[name='chkBook']:checked").attr("data-cardinal");
	...
	$("#bookPaymentForm input[name='learn_app_id']").val(learn_app_id);
	$("#bookPaymentForm input[name='course_id']").val(course_id);
	$("#bookPaymentForm input[name='cardinal_id']").val(cardinal_id);
	...
	$.ajax({
		...
		success:function(data) {
			...
			for (var i = 0; i < itemList.length; i++) {
				var isMain = itemList[i].MAIN_YN == "Y";

				var etc = isMain?"본교재":"교구";
				var price = isMain?itemList[i].MAIN_PRICE:itemList[i].SUB_PRICE;

				var book = '<li class="list-item">';
				book += '<span class="item-title">' + itemList[i].COURSE_NAME + ' ' + etc + ' -  ' + itemList[i].NAME  + '</span>';
				book += '<span class="item-price"><strong>' + addComma(price) + '</strong> 원</span>';
				book += '<span class="item-buy"><input type="number" class="w4em" value="0" name="book" book="'+itemList[i].ID+'" total="0" price="'+price+'" max="'+itemList[i].STOCK+'" min="0"> 권</span>';
				book += '<span class="item-max"><strong>' + addComma(itemList[i].STOCK) + '</strong> 권</span>';
				book += '</li>';

				$("#bookListFooter").before(book);
			}

			//주문관련
			$("#bookPaymentForm input[name='order_idx']").val(data.order_idx);

			bookEventReassign();
			popupOpen("orderBookPop");
		}
	});
...
<!-- 1.3. 본문 -->
				...
				<div class="sub-body">
					...
					<div class="tab-group">
						<ul class="tab-header sub x3">
							<li><a class="nth-1st on" href="lecture1">진행 연수<strong class="num">${progressList.size() }</strong></a></li>
						...
						<div class="tab-body">
									...
									<c:if test="${!empty progressList }">
										<c:forEach var="item" items="${progressList}" varStatus="status">
												<tr class="<c:if test="${status.index == 0 }">nth-1st</c:if>"><!-- ie8에서는 nth-first 선택자를 이용할 수 없으므로 첫번째 아이템에만 class 명으로 부여 -->
													<td class="item-radio">
														<c:if test="${item.MAIN_TB_ID ne 0 ||  item.SUB_TB_ID ne 0 }">
															<input type="radio" name="chkBook" value="${item.ID }" data-cadinal="${item.CARDINAL_ID }" data-course="${item.COURSE_ID }">
														</c:if>
													</td>
													...
													<td class="item-function"><a class="btn primary" href="classroom?id=${item.ID }&cardinal=${item.CARDINAL_ID }&course=${item.COURSE_ID }"  target="_self">강의실 입장</a></td>
												</tr>
										</c:forEach>
									</c:if>
									<c:if test="${empty progressList }">
							...
							<div class="list-bottom alignR">
								<a href="javascript:orderBook();" class="btn danger">교재 구매</a>
							</div>
						</div><!--//.tab-body-->
					</div><!--.tab-group-->
					...
					<div class="section box">
						<div class="section-header">
							...
							<p class="ctrl-btnSet">
									<a class="btn-modify" href="/mypage/private/privatecertification">개인 정보 수정</a>
								</p>
						</div>
						<div class="section-body borderT">
							<ul class="info-list">
	...
	<t:insertDefinition name="orderbook"/>										<!-- orderbook popup : 교재 구매 -->